...
object FrenchStreamProvider : Provider, ProviderPortalUrl, ProviderConfigUrl {
    ...
    override suspend fun getHome(): List<Category> {
        ...
        if ( cookie.contains("VFV25")) {
            ...
            document.select("section.vod-section").map { cat_item ->
                val title = cat_item
                    .select("> div.vod-header h2.vod-title-section").firstOrNull()
                    ?.let {
                        listOfNotNull(
                            it.ownText().trim(),
                ...
                val movies = cat_item
                    ...
                    .mapNotNull { item ->
                        val a = item.select("a").firstOrNull() ?: return@mapNotNull null
                        val link = a.attr("href")
                        val href = link.substringAfterLast("/")
                        val title = a.select("div.vod-name").firstOrNull()?.text() ?: ""
                        val poster = a.select("div.vod-poster > img").firstOrNull()?.attr("src") ?: ""
                        val mtype = item.select("> div.vod-br > span.vod-tag a").firstOrNull()?.attr("href") ?: ""
            ...
            categories.add(
                Category(
                    ...
                    list = document.select("div.pages.clearfix").getOrNull(1)?.select("div.short")
                        ?.map {
                            TvShow(
                                id = it.select("a.short-poster").firstOrNull()
                                    ?.attr("href")?.substringAfterLast("/")
                                    ?: "",
                                title = listOfNotNull(
                                    it.select("div.short-title").firstOrNull()?.text(),
                                    it.select("span.film-version").firstOrNull()?.text(),
                                ).joinToString(" - "),
                                poster = it.select("img").firstOrNull()
                                    ?.attr("src")
                                    ?: "",
                            )
                        } ?: emptyList(),
                        ...
                        ?.map {
                            Movie(
                                id = it.select("a.short-poster").firstOrNull()
                                    ?.attr("href")?.substringAfterLast("/")
                                    ?: "",
                                title = it.select("div.short-title").firstOrNull()
                                    ?.text()
                                    ?: "",
                                poster = it.select("img").firstOrNull()
                                    ?.attr("src")
    ...
    override suspend fun search(query: String, page: Int): List<AppAdapter.Item> {
        ...
        if (query.isEmpty()) {
            ...
            val genres = document.select("div.menu-section").firstOrNull()?.select(">a")?.map {
                Genre(
        ...
        val results = document.select("div.search-item")
            .mapNotNull {
                ...
                val title = it.select("div.search-title").firstOrNull()
                    ?.text()?.replace("\\'", "'")
                    ?: ""
                var poster = it.select("img").firstOrNull()
                    ?.attr("src")
                    ?: ""
    ...
    suspend fun getRating(votes: Element?): Double {
        if (votes == null) return 0.0
        val voteplus = votes
            .select("span.ratingtypeplusminus").firstOrNull()
            ?.text()
            ?.toIntOrNull() ?: 0
    ...
    override suspend fun getMovie(id: String): Movie {
        ...
        val votes = document.select("div.fr-votes").firstOrNull()
        val rating = getRating(votes)
        val movie = Movie(
            id = id,
            title = document.select("meta[property=og:title]").firstOrNull()?.attr("content")
                ?: "",
            overview = document.select("div#s-desc").firstOrNull()
                ?.apply {
                    select("p.desc-text").firstOrNull()?.remove()
                }
                ?.text()
                ?.trim()
                ?: "",
            released = document.select("span.release_date").firstOrNull()
                ?.text()
                ?.substringAfter("-")
                ?.trim(),
            ...
            quality = document.select("span[id=film_quality]").firstOrNull()
                ?.text(),
            poster = poster,
            banner = banner,
            trailer = trailerURL,
            ...
            directors = document.select("ul#s-list li")
                .find {
                    it.select("span").firstOrNull()?.text()?.contains("alisateur") == true
                }
    ...
    override suspend fun getTvShow(id: String): TvShow {
        ...
        val votes = document.select("div.fr-votes").firstOrNull()
        val rating = getRating(votes)
        val title = document.select("meta[property=og:title]").firstOrNull()
            ?.attr("content")
            ?: ""
        ...
        val tvShow = TvShow(
            ...
            overview = document.select("div.fdesc > p").firstOrNull()
                ?.text()
                ?.trim()
                ?: "",
            released = document.select("span.release").firstOrNull()
                ?.text()
                ?.substringBefore("-")
                ?.trim(),
            runtime = document.select("span.runtime")
    ...
    override suspend fun getPeople(id: String, page: Int): People {
        ...
        val people = People(
            ...
            filmography = document.select("div#dle-content > div.short").mapNotNull {
                val href = it.select("a.short-poster").firstOrNull()
                    ?.attr("href")
                    ?: ""
                val id = href.substringAfterLast("/")

                val title = it.select("div.short-title").firstOrNull()
                    ?.text()
                    ?: ""
                val poster = it.select("img").firstOrNull()
                    ?.attr("src")
                    ?: ""
            ...
            is Video.Type.Episode -> {
                ...
                episodesData?.vf?.get(tvShowNumber)?.forEach { (provider, url) ->
                    tvShowServers.add(Video.Server(
                        id = "vf$provider",
                        name = provider.replaceFirstChar { if (it.isLowerCase()) it.titlecase(Locale.getDefault()) else it.toString() }+" (VF)",
                        src = url
                    ))
                }
                episodesData?.vostfr?.get(tvShowNumber)?.forEach { (provider, url) ->
                    tvShowServers.add(Video.Server(
                        id = "vostfr$provider",
                        name = provider.replaceFirstChar { if (it.isLowerCase()) it.titlecase(Locale.getDefault()) else it.toString() }+" (VOSTFR)",
                        src = url
                    ))
                }
                episodesData?.vo?.get(tvShowNumber)?.forEach { (provider, url) ->
                    tvShowServers.add(Video.Server(
                        id = "vo$provider",
                        name = provider.replaceFirstChar { if (it.isLowerCase()) it.titlecase(Locale.getDefault()) else it.toString() }+" (VO)",
                        src = url
                    ))
                }
                tvShowServers
                    ...
                    filmData.players.forEach { (provider, langMap) ->
                            ...
                            .forEach { (lang, url) ->
                                ...
                                if (url.startsWith("http") || url.isNotBlank()) {
                                    ...
                                    if (seenUrlsForProvider.add(url) && !ignoreSource(provider, url)) {
                                        ...
                                        val displayName =
                                            provider.replaceFirstChar { if (it.isLowerCase()) it.titlecase(Locale.getDefault()) else it.toString() } +
                        ...
                        if (!defaultUrl.isNullOrBlank()
                            ...
                            movieServers.add(
                                Video.Server(
                                    id = "vid${serverIndex++}",
                                    name = provider.replaceFirstChar { if (it.isLowerCase()) it.titlecase(Locale.getDefault()) else it.toString() },
                                    src = defaultUrl
                                )
                            )
                        }
                ...
                try {
                    ...
                    if (!newUrl.isNullOrEmpty()) {
                        ...
                        UserPreferences.setProviderCache(
                            ...
                            formattedUrl + "favicon-96x96.png"
                        )
                    }
                } catch (e: Exception) {
                }
