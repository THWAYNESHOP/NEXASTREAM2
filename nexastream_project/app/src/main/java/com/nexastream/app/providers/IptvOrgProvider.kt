            ...
            body.source().use { source ->
                ...
                while (!source.exhausted()) {
                    ...
                    if (line.startsWith("#EXTINF")) {
                        ...
                        curLogo = Regex("""tvg-logo="([^"]+)"""").find(line)?.groupValues?.get(1) ?: ""
                        curGroup = Regex("""group-title="([^"]+)"""").find(line)?.groupValues?.get(1) ?: ""
                        curUA = Regex("""http-user-agent="([^"]+)"""").find(line)?.groupValues?.get(1)
                    } else if (line.startsWith("#EXTVLCOPT:http-user-agent=")) {
                        curUA = line.substringAfter("http-user-agent=").trim()
                    } else if (line.startsWith("http")) {
                        if (curName.isNotEmpty()) {
                            channels.add(M3UChannel(curName, line, curLogo, curGroup, curUA))
