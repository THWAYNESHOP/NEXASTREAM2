...
class SearchTvFragment : Fragment() {
            ...
            kotlinx.coroutines.flow.combine(
                ...
                if (viewModel.query.isEmpty()) {
                    ...
                    if (historyList.isNotEmpty()) {
                        val recentItems = historyList.map { searchHist ->
                    ...
                    if (trendingList.isNotEmpty()) {
                        categories.add(Category("Trending Now", trendingList.onEach {
                            when (it) {
                    ...
                    if (topRatedList.isNotEmpty()) {
                        categories.add(Category("Top Rated", topRatedList.onEach {
                            when (it) {
                                is Movie -> it.itemType = AppAdapter.Type.MOVIE_TV_ITEM
                                is TvShow -> it.itemType = AppAdapter.Type.TV_SHOW_TV_ITEM
                            }
                    ...
                    if (airingTodayList.isNotEmpty()) {
                        categories.add(Category("Airing Today", airingTodayList.onEach {
                            when (it) {
                                is Movie -> it.itemType = AppAdapter.Type.MOVIE_TV_ITEM
                                is TvShow -> it.itemType = AppAdapter.Type.TV_SHOW_TV_ITEM
                            }
                ...
                if (viewModel.query.isEmpty() && categories.isNotEmpty()) {
    ...
    private fun displaySearch(list: List<AppAdapter.Item>, hasMore: Boolean) {
        currentGridColumns = if (viewModel.query == "") 1 else 6
        binding.vgvSearch.setNumColumns(currentGridColumns)
