...
inline fun <reified T : ViewModel> Fragment.activityViewModelsFactory(crossinline viewModelInitialization: () -> T): Lazy<T> {
...
private val MediaMetadata.Builder.extras: Bundle?
    get() = try {
        val field = this.javaClass.getDeclaredField("extras")
...
val MediaMetadata.mediaServerId: String?
    get() = extras?.getString("mediaServerId")

val MediaMetadata.mediaServers: List<MediaServer>
    get() = extras?.run {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            getParcelableArray("mediaServers", MediaServer::class.java)?.toList()
        } else {
            @Suppress("DEPRECATION")
            getParcelableArray("mediaServers")?.map { it as MediaServer }
        }
    } ?: emptyList()

fun MediaMetadata.Builder.setMediaServerId(mediaServerId: String) = this
    .setExtras((extras ?: Bundle()).also { bundle ->
