...
object FrenchAnimeProvider : Provider, ProviderConfigUrl {
    ...
    override suspend fun getHome(): List<Category> {
        ...
        document.select(".owl-carousel .item").map { item ->
            val a = item.select("a").firstOrNull() ?: return@map null
            val link = a.attr("href")

            TvShow(
                id = link.substringAfterLast("/").substringBefore(".html"),
                title = a.select(".title1").firstOrNull()?.text() ?: "",
                overview = a.select(".title0").firstOrNull()?.text(),
                banner = a.select("img").firstOrNull()?.attr("src")?.toUrl()
            )
        ...
        document.select(".block-main").forEach { block ->
            val categoryName = block.select(".block-title .left-ma").firstOrNull()?.text() ?: ""
            block.select(".mov").mapNotNull { mov ->
                val a = mov.select("a.mov-t").firstOrNull() ?: return@mapNotNull null
                val link = a.attr("href")
                val id = link.substringAfterLast("/").substringBefore(".html")
                val isFrench = (mov.select(".block-sai").firstOrNull()?.text()
                    ?: mov.select(".nbloc1").firstOrNull()?.text()).isFrench()
                val title = if (categoryName.contains("FILMS", ignoreCase = true))
                    a.text().toTitle(isFrench) else a.text()
                val poster = mov.select(".mov-i img").firstOrNull()?.attr("src")?.toUrl() ?: ""

                val isTvShow = mov.select(".block-ep").firstOrNull() != null
                if (isTvShow) {
                    val seasonText = mov.select(".block-sai").firstOrNull()?.text() ?: ""
                    val seasonNumber = seasonText.split(" ").getOrNull(1)?.toIntOrNull() ?: 0
                    val episodeText = mov.select(".block-ep").firstOrNull()?.text() ?: ""
    ...
    override suspend fun search(query: String, page: Int): List<AppAdapter.Item> {
        ...
        document.select("div.berrors").firstOrNull()?.let(::checkHasMore)

        val results = document.select("div.mov").mapNotNull { mov ->
            val a = mov.select("a.mov-t").firstOrNull() ?: return@mapNotNull null
            val link = a.attr("href")
            val id = link.substringAfterLast("/").substringBefore(".html")
            val isFrench = (mov.select(".block-sai").firstOrNull()?.text() ?: mov.select(".nbloc1").firstOrNull()
                ?.text()).isFrench()
    ...
    override suspend fun getMovies(page: Int): List<Movie> {
        ...
        val movies = document.select("div.mov.clearfix").mapNotNull { mov ->
            val a = mov.select("a.mov-t").firstOrNull() ?: return@mapNotNull null
            val link = a.attr("href")
            val isFrench = mov.select(".nbloc1").firstOrNull()?.text().isFrench()

            Movie(
                id = link.substringAfterLast("/").substringBefore(".html"),
                title = a.text().toTitle(isFrench),
                poster = mov.select(".mov-i img").firstOrNull()?.attr("src")?.toUrl()
            )
        }
    ...
    override suspend fun getMovie(id: String): Movie {
        ...
        val isFrench = document.select("ul.mov-list li:contains(Version) .mov-desc span").firstOrNull()
            ?.text().isFrench()

        val movie = Movie(
            id = id,
            title = document.select("header.full-title h1").firstOrNull()?.text()?.toTitle(isFrench) ?: "",
            overview = document.select("span[itemprop='description']").firstOrNull()?.text() ?: "",
            released = document.select("ul.mov-list li")
                .find { it.select("div.mov-label").firstOrNull()?.text() == "Date de sortie:" }
                ?.select("div.mov-desc")?.firstOrNull()?.text()?.substringBefore(" to"),
            runtime = document.select("ul.mov-list li")
                .find { it.select("div.mov-label").firstOrNull()?.text() == "Durée:" }
                ?.select("div.mov-desc")?.firstOrNull()?.text()?.extractRuntime(),
            poster = document.select("div.mov-img img[itemprop='thumbnailUrl']").firstOrNull()
                ?.attr("src")?.toUrl(),
            genres = document.select("ul.mov-list li")
                .find { it.select("div.mov-label").firstOrNull()?.text() == "GENRE:" }
            ...
            directors = document.select("ul.mov-list li")
                .find { it.select("div.mov-label").firstOrNull()?.text() == "RÉALISATEUR:" }
                ?.select("div.mov-desc span[itemprop='name']")?.firstOrNull()?.text()
                .toPeople(),
            cast = document.select("ul.mov-list li")
                .find { it.select("div.mov-label").firstOrNull()?.text() == "ACTEURS:" }
                ?.select("div.mov-desc span[itemprop='name']")?.firstOrNull()?.text()
                .toPeople()
        )
    ...
    override suspend fun getTvShow(id: String): TvShow {
        ...
        val tvShow = TvShow(
            id = id,
            title = document.select("h1[itemprop=name]").firstOrNull()?.text()?.toTitle(isFrench) ?: "",
            overview = document.select("span[itemprop=description]").firstOrNull()?.text(),
            released = document.select("ul.mov-list li")
                .find { it.select("div.mov-label").firstOrNull()?.text() == "Date de sortie:" }
                ?.select("div.mov-desc")?.firstOrNull()?.text()?.substringBefore(" to"),
            runtime = document.select("ul.mov-list li")
                .find { it.select("div.mov-label").firstOrNull()?.text() == "Durée:" }
                ?.select("div.mov-desc")?.firstOrNull()?.text()?.extractRuntime(),
            poster = document.select("div.mov-img img[itemprop='thumbnailUrl']").firstOrNull()?.attr("src")
                ?.toUrl(),
            seasons = listOf(
                Season(
                    id = id,
    ...
    override suspend fun getEpisodesBySeason(seasonId: String): List<Episode> {
        ...
        val epsText = document.select("div.eps").firstOrNull()?.text() ?: return emptyList()
    ...
    private fun extractTvShows(mov: Element): TvShow? {
        val a = mov.select("a.mov-t").firstOrNull() ?: return null
        val link = a.attr("href")
        val id = link.substringAfterLast("/").substringBefore(".html")
        val isFrench = (mov.select(".block-sai").firstOrNull()?.text() ?: mov.select(".nbloc1").firstOrNull()
            ?.text()).isFrench()
        val title = a.text().toTitle(isFrench)
        val poster = mov.select(".mov-i img").firstOrNull()?.attr("src")?.toUrl() ?: ""
        val episodeText = mov.select(".block-ep").firstOrNull()?.text() ?: return null
