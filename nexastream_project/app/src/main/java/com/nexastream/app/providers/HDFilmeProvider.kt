...
object HDFilmeProvider : Provider {
    ...
    override val baseUrl = "https://hdfilme.win"
    override val language = "de"
    override val logo = "$baseUrl/templates/hdfilme/images/apple-touch-icon.png"
    ...
    override suspend fun getMovies(page: Int): List<Movie> {
        val url = if (page == 1) "$baseUrl/filme1/" else "$baseUrl/filme1/page/$page"
        val document = service.getPage(url)
        return parseShows(document.select("article")).filterIsInstance<Movie>()
    }

    override suspend fun getTvShows(page: Int): List<TvShow> {
        val url = if (page == 1) "$baseUrl/serien/" else "$baseUrl/serien/page/$page"
        val document = service.getPage(url)
    ...
    override suspend fun getTvShow(id: String): TvShow {
        ...
        document.select("div.seasons a").forEach { link ->
            ...
            val seasonNumber = Regex("""staffel-(\d+)""").find(href)?.groupValues?.getOrNull(1)?.toIntOrNull()
                ?: return@forEach
    ...
    override suspend fun getEpisodesBySeason(seasonId: String): List<Episode> {
        ...
        return document.select("div.episodes a").mapNotNull { link ->
            val episodeNumber = Regex("""folge-(\d+)""").find(link.attr("href"))?.groupValues?.getOrNull(1)?.toIntOrNull()
