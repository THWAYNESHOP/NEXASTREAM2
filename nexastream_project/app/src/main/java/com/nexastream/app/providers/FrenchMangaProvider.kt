...
object FrenchMangaProvider : Provider, ProviderPortalUrl, ProviderConfigUrl {
    ...
    override suspend fun getHome(): List<Category> {
        ...
        document.select("div.sect").mapNotNull { cat_item ->
            val title = cat_item.select("div.st-left > a").firstOrNull()
            if (title == null) return@mapNotNull null

            val movies = cat_item.select("div.short > div.short-in").mapNotNull { movie ->
                val mtype = movie.select("span.mli-type a").firstOrNull()?.text()
                if (mtype == null) return@mapNotNull null
                val title = movie.select("div.short-title").firstOrNull()?.text() ?: "Item"
                val href = movie.select("a.short-poster").firstOrNull()
                val poster = href?.select("> img")?.firstOrNull()?.attr("src")
    ...
    override suspend fun getMovie(id: String): Movie {
        ...
        val md = document.select("div#manga-data").firstOrNull()
        ...
        val movie = Movie( id = md.attr("data-newsid"),
            ...
            released = document.select("span.release").firstOrNull()?.text(),
            overview = document.select("div.fdesc").firstOrNull()?.text(),
            ...
            directors = document.select("ul#s-list li")
                .find {
                    it.select("span").firstOrNull()?.text()?.contains("Director:") == true
    ...
    fun getRating(votes: Element): Double {
        val voteplus = votes.select("span.ratingtypeplusminus").firstOrNull()
            ?.text()
            ?.toIntOrNull() ?: 0
            ...
            is Video.Type.Episode -> {
                ...
                episodesData?.vf?.get(tvShowNumber)?.forEach { (provider, url) ->
                    tvShowServers.add(Video.Server(
                        id = "vf$provider",
                        name = provider.replaceFirstChar { if (it.isLowerCase()) it.titlecase() else it.toString() }+" (VF)",
                        src = url
                    ))
                }
                episodesData?.vostfr?.get(tvShowNumber)?.forEach { (provider, url) ->
                    tvShowServers.add(Video.Server(
                        id = "vostfr$provider",
                        name = provider.replaceFirstChar { if (it.isLowerCase()) it.titlecase() else it.toString() }+" (VOSTFR)",
                        src = url
                    ))
                }
                episodesData?.vo?.get(tvShowNumber)?.forEach { (provider, url) ->
                    tvShowServers.add(Video.Server(
                        id = "vo$provider",
                        name = provider.replaceFirstChar { if (it.isLowerCase()) it.titlecase() else it.toString() }+" (VO)",
                        src = url
                    ))
                }
                tvShowServers
                    ...
                    map?.values?.forEach { servers ->
                        servers.forEach { (provider, url) ->
                            ...
                            movieServers.add(Video.Server(
                                ...
                                name = provider.replaceFirstChar { if (it.isLowerCase()) it.titlecase() else it.toString() }+" ($lang)$suffix",
                                src = url
                            ))
                        }
                    }
            ...
            if (forceRefresh || UserPreferences.getProviderCache(this,UserPreferences.PROVIDER_AUTOUPDATE) != "false") {
                ...
                try {
                    ...
                    if (!fsUrl.isNullOrEmpty()) {
                        ...
                        var newUrl = fsdoc
                            .select("li.submenu:has(a:contains(ANIMES)) a").firstOrNull()
                            ?.attr("href")
        ...
        companion object {
            ...
            fun buildAddressFetcher(): FrenchMangaService {
                val addressRetrofit = Retrofit.Builder()
                    .baseUrl(FrenchMangaProvider.portalUrl)
