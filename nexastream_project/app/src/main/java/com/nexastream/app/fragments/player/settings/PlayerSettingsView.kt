...
import com.nexastream.app.utils.mediaServers
import com.nexastream.app.utils.SubDL
import com.nexastream.app.utils.UserPreferences
import com.nexastream.app.utils.dp
import com.nexastream.app.utils.findClosest
import com.nexastream.app.utils.getAlpha
import com.nexastream.app.utils.getRgb
import com.nexastream.app.utils.mediaServerId
import com.nexastream.app.utils.mediaServers
import com.nexastream.app.utils.setAlpha
import com.nexastream.app.utils.setRgb
import com.nexastream.app.utils.supportedTrackFormats
...
abstract class PlayerSettingsView @JvmOverloads constructor(
    ...
    protected var onSubtitleOffsetSelected: ((Settings.Subtitle.Offset.OffsetItem) -> Unit) =
        fun(offset) {
            UserPreferences.subtitleOffset = offset.value
            subtitleOffsetSelectionListener?.invoke(offset)
        }

    protected var subtitleOffsetSelectionListener: ((Settings.Subtitle.Offset.OffsetItem) -> Unit)? = null
    fun setOnSubtitleOffsetSelectedListener(listener: (Settings.Subtitle.Offset.OffsetItem) -> Unit) {
        subtitleOffsetSelectionListener = listener
    }
    ...
    sealed class Settings : Item {
        ...
        sealed class Subtitle : Item {
            ...
            sealed class Offset : Item {
                companion object : Settings.Subtitle() {
                    val list: List<Item> = (-100..100).map { OffsetItem(it * 100L) }
                    val selected: OffsetItem
                        get() = list.filterIsInstance<OffsetItem>().find { it.value == UserPreferences.subtitleOffset }
                            ?: list.filterIsInstance<OffsetItem>().find { it.value == 0L }
                            ?: OffsetItem(0L)
                }

                class OffsetItem(val value: Long) : Offset()
            }
            ...
            sealed class OpenSubtitles : Item {
                ...
                companion object : Settings.Subtitle() {
                    ...
                    fun init(openSubtitles: List<com.nexastream.app.utils.OpenSubtitles.Subtitle>) {
                        list.clear()
                ...
                class Subtitle(
                    val openSubtitle: com.nexastream.app.utils.OpenSubtitles.Subtitle
                ) : OpenSubtitles()
            }
            ...
            sealed class SubDLSubtitles : Item {
                ...
                companion object : Settings.Subtitle() {
                    ...
                    fun init(subDLSubtitles: List<com.nexastream.app.utils.SubDL.Subtitle>) {
                        list.clear()
                ...
                class Subtitle(
                    val subDLSubtitle: com.nexastream.app.utils.SubDL.Subtitle
                ) : SubDLSubtitles()
            }
        }
